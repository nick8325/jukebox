module Data.Derive.BinaryUnshared where
{-
import BinaryShared
import Data.Word
...
instance Binary alpha => BinaryUnshared (Sample alpha) where
    putUnshared x = case x of
        First          -> do putTag 0
    ...
    getUnshared = do
        ...
        where
            useTag = length [First{}, Second{}, Third{}] > 1
            getTag = if useTag then getWord8 else return (0 :: Word8)
-}
-- GENERATED START
...
makeBinaryUnshared :: Derivation
makeBinaryUnshared = derivationDSL "BinaryUnshared" dslBinaryUnshared

dslBinaryUnshared =
    List [Instance ["Binary"] "BinaryUnshared" (List [App "InsDecl" (
    List [App "FunBind" (List [List [App "Match" (List [App "Ident" (
    List [String "putUnshared"]),List [App "PVar" (List [App "Ident" (
    List [String "x"])])],App "Nothing" (List []),App "UnGuardedRhs" (
    ...
    String "x",ShowInt FieldIndex])])]))]),App "UnGuardedAlt" (List [
    App "Do" (List [Concat (List [List [App "Qualifier" (List [App
    "App" (List [App "Var" (List [App "UnQual" (List [App "Ident" (
    List [String "putTag"])])]),App "Lit" (List [App "Int" (List [
    CtorIndex])])])])],MapField (App "Qualifier" (List [App "App" (
    List [App "Var" (List [App "UnQual" (List [App "Ident" (List [
    String "put"])])]),App "Var" (List [App "UnQual" (List [App
    ...
    List [String "useTag"])]),App "Nothing" (List []),App
    ...
    App "BDecls" (List [List []])]),App "FunBind" (List [List [App
    "Match" (List [App "Ident" (List [String "putTag"]),List [App
    ...
    List [App "Ident" (List [String "x"])])])])])])]),App "BDecls" (
    List [List []])])]])]])])]])]),App "InsDecl" (List [App "PatBind"
    (List [App "PVar" (List [App "Ident" (List [String "getUnshared"])
    ...
    "App" (List [App "Var" (List [App "UnQual" (List [App "Ident" (
    List [String "return"])])]),App "Paren" (List [App "ExpTypeSig" (
    List [App "Lit" (List [App "Int" (List [Int 0])]),App "TyCon" (
    List [App "UnQual" (List [App "Ident" (List [Concat (List [String
    "Word",ShowInt (Int 8)])])])])])])])])]),App "BDecls" (List [List
    []])])]])])])])]
-- GENERATED STOP
